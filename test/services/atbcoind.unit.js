...
var bitcore = require('atbcore-lib');
...
var BitcoinService = proxyquire('../../lib/services/atbcoind', {
  fs: {
...
describe('Bitcoin Service', function() {
  ...
  describe('@constructor', function() {
    it('will create an instance', function() {
      var atbcoind = new BitcoinService(baseConfig);
      should.exist(atbcoind);
    });
    it('will create an instance without `new`', function() {
      var atbcoind = BitcoinService(baseConfig);
      should.exist(atbcoind);
    });
    it('will init caches', function() {
      var atbcoind = new BitcoinService(baseConfig);
      should.exist(atbcoind.utxosCache);
      should.exist(atbcoind.txidsCache);
      should.exist(atbcoind.balanceCache);
      should.exist(atbcoind.summaryCache);
      should.exist(atbcoind.transactionDetailedCache);

      should.exist(atbcoind.transactionCache);
      should.exist(atbcoind.rawTransactionCache);
      should.exist(atbcoind.blockCache);
      should.exist(atbcoind.rawBlockCache);
      should.exist(atbcoind.blockHeaderCache);
      should.exist(atbcoind.zmqKnownTransactions);
      should.exist(atbcoind.zmqKnownBlocks);
      should.exist(atbcoind.lastTip);
      should.exist(atbcoind.lastTipTimeout);
    });
    it('will init clients', function() {
      var atbcoind = new BitcoinService(baseConfig);
        atbcoind.nodes.should.deep.equal([]);
        atbcoind.nodesIndex.should.equal(0);
        atbcoind.nodes.push({client: sinon.stub()});
      should.exist(atbcoind.client);
    });
    it('will set subscriptions', function() {
      var atbcoind = new BitcoinService(baseConfig);
        atbcoind.subscriptions.should.deep.equal({
        address: {},
        rawtransaction: [],
        hashblock: []
      });
  ...
  describe('#_initDefaults', function() {
    it('will set transaction concurrency', function() {
      var atbcoind = new BitcoinService(baseConfig);
        atbcoind._initDefaults({transactionConcurrency: 10});
        atbcoind.transactionConcurrency.should.equal(10);
        atbcoind._initDefaults({});
        atbcoind.transactionConcurrency.should.equal(5);
    });
  });
  ...
  describe('#getAPIMethods', function() {
    it('will return spec', function() {
      var atbcoind = new BitcoinService(baseConfig);
      var methods = atbcoind.getAPIMethods();
  ...
  describe('#getPublishEvents', function() {
    it('will return spec', function() {
      var atbcoind = new BitcoinService(baseConfig);
      var events = atbcoind.getPublishEvents();
      should.exist(events);
      events.length.should.equal(3);
      events[0].name.should.equal('atbcoind/rawtransaction');
      events[0].scope.should.equal(atbcoind);
      events[0].subscribe.should.be.a('function');
      events[0].unsubscribe.should.be.a('function');
      events[1].name.should.equal('atbcoind/hashblock');
      events[1].scope.should.equal(atbcoind);
      events[1].subscribe.should.be.a('function');
      events[1].unsubscribe.should.be.a('function');
      events[2].name.should.equal('atbcoind/addresstxid');
      events[2].scope.should.equal(atbcoind);
    ...
    it('will call subscribe/unsubscribe with correct args', function() {
      var atbcoind = new BitcoinService(baseConfig);
        atbcoind.subscribe = sinon.stub();
        atbcoind.unsubscribe = sinon.stub();
      var events = atbcoind.getPublishEvents();

      events[0].subscribe('test');
        atbcoind.subscribe.args[0][0].should.equal('rawtransaction');
        atbcoind.subscribe.args[0][1].should.equal('test');

      events[0].unsubscribe('test');
        atbcoind.unsubscribe.args[0][0].should.equal('rawtransaction');
        atbcoind.unsubscribe.args[0][1].should.equal('test');

      events[1].subscribe('test');
        atbcoind.subscribe.args[1][0].should.equal('hashblock');
        atbcoind.subscribe.args[1][1].should.equal('test');

      events[1].unsubscribe('test');
        atbcoind.unsubscribe.args[1][0].should.equal('hashblock');
        atbcoind.unsubscribe.args[1][1].should.equal('test');
    });
  });
  ...
  describe('#subscribe', function() {
    ...
    it('will push to subscriptions', function() {
      var atbcoind = new BitcoinService(baseConfig);
      var emitter = {};
        atbcoind.subscribe('hashblock', emitter);
        atbcoind.subscriptions.hashblock[0].should.equal(emitter);

      var emitter2 = {};
        atbcoind.subscribe('rawtransaction', emitter2);
        atbcoind.subscriptions.rawtransaction[0].should.equal(emitter2);
    });
  });
  ...
  describe('#unsubscribe', function() {
    ...
    it('will remove item from subscriptions', function() {
      ...
      var emitter4 = {};
      var emitter5 = {};
        atbcoind.subscribe('hashblock', emitter1);
        atbcoind.subscribe('hashblock', emitter2);
        atbcoind.subscribe('hashblock', emitter3);
        atbcoind.subscribe('hashblock', emitter4);
        atbcoind.subscribe('hashblock', emitter5);
        atbcoind.subscriptions.hashblock.length.should.equal(5);

        atbcoind.unsubscribe('hashblock', emitter3);
        atbcoind.subscriptions.hashblock.length.should.equal(4);
        atbcoind.subscriptions.hashblock[0].should.equal(emitter1);
        atbcoind.subscriptions.hashblock[1].should.equal(emitter2);
        atbcoind.subscriptions.hashblock[2].should.equal(emitter4);
        atbcoind.subscriptions.hashblock[3].should.equal(emitter5);
    });
    it('will not remove item an already unsubscribed item', function() {
      var atbcoind = new BitcoinService(baseConfig);
      var emitter1 = {};
      var emitter3 = {};
        atbcoind.subscriptions.hashblock= [emitter1];
        atbcoind.unsubscribe('hashblock', emitter3);
        atbcoind.subscriptions.hashblock.length.should.equal(1);
        atbcoind.subscriptions.hashblock[0].should.equal(emitter1);
    });
  });
  ...
  describe('#subscribeAddress', function() {
    ...
    it('will not an invalid address', function() {
      ...
      var emitter = new EventEmitter();
        atbcoind.subscribeAddress(emitter, ['invalidaddress']);
      should.not.exist(atbcoind.subscriptions.address['invalidaddress']);
    });
    it('will add a valid address', function() {
      var atbcoind = new BitcoinService(baseConfig);
      var emitter = new EventEmitter();
        atbcoind.subscribeAddress(emitter, ['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br']);
      should.exist(atbcoind.subscriptions.address['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br']);
    });
    it('will handle multiple address subscribers', function() {
      var atbcoind = new BitcoinService(baseConfig);
      var emitter1 = new EventEmitter();
      var emitter2 = new EventEmitter();
        atbcoind.subscribeAddress(emitter1, ['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br']);
        atbcoind.subscribeAddress(emitter2, ['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br']);
      should.exist(atbcoind.subscriptions.address['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br']);
        atbcoind.subscriptions.address['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br'].length.should.equal(2);
    });
    it('will not add the same emitter twice', function() {
      ...
      var emitter1 = new EventEmitter();
        ...
        atbcoind.subscribeAddress(emitter1, ['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br']);
      should.exist(atbcoind.subscriptions.address['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br']);
        atbcoind.subscriptions.address['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br'].length.should.equal(1);
    });
  });
  ...
  describe('#unsubscribeAddress', function() {
    ...
    it('it will remove a subscription', function() {
      ...
      should.exist(atbcoind.subscriptions.address['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br']);
        atbcoind.subscriptions.address['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br'].length.should.equal(2);
        atbcoind.unsubscribeAddress(emitter1, ['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br']);
        atbcoind.subscriptions.address['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br'].length.should.equal(1);
    });
    it('will unsubscribe subscriptions for an emitter', function() {
      ...
      var emitter1 = new EventEmitter();
      var emitter2 = new EventEmitter();
        atbcoind.subscriptions.address['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br'] = [emitter1, emitter2];
        atbcoind.unsubscribeAddress(emitter1);
        atbcoind.subscriptions.address['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br'].length.should.equal(1);
    });
    it('will NOT unsubscribe subscription with missing address', function() {
      ...
      var emitter2 = new EventEmitter();
        atbcoind.subscriptions.address['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br'] = [emitter1, emitter2];
        atbcoind.unsubscribeAddress(emitter1, ['1Cj4UZWnGWAJH1CweTMgPLQMn26WRMfXmo']);
        atbcoind.subscriptions.address['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br'].length.should.equal(2);
    });
    it('will NOT unsubscribe subscription with missing emitter', function() {
      ...
      var emitter1 = new EventEmitter();
      var emitter2 = new EventEmitter();
        atbcoind.subscriptions.address['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br'] = [emitter2];
        atbcoind.unsubscribeAddress(emitter1, ['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br']);
        atbcoind.subscriptions.address['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br'].length.should.equal(1);
        atbcoind.subscriptions.address['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br'][0].should.equal(emitter2);
    });
    it('will remove empty addresses', function() {
      ...
      var emitter2 = new EventEmitter();
        atbcoind.subscriptions.address['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br'] = [emitter1, emitter2];
        atbcoind.unsubscribeAddress(emitter1, ['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br']);
        atbcoind.unsubscribeAddress(emitter2, ['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br']);
      should.not.exist(atbcoind.subscriptions.address['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br']);
    });
    it('will unsubscribe emitter for all addresses', function() {
      ...
      var emitter2 = new EventEmitter();
        atbcoind.subscriptions.address['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br'] = [emitter1, emitter2];
        atbcoind.subscriptions.address['1Cj4UZWnGWAJH1CweTMgPLQMn26WRMfXmo'] = [emitter1, emitter2];
      sinon.spy(atbcoind, 'unsubscribeAddressAll');
        atbcoind.unsubscribeAddress(emitter1);
        atbcoind.unsubscribeAddressAll.callCount.should.equal(1);
        atbcoind.subscriptions.address['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br'].length.should.equal(1);
        atbcoind.subscriptions.address['1Cj4UZWnGWAJH1CweTMgPLQMn26WRMfXmo'].length.should.equal(1);
    });
  });
  ...
  describe('#unsubscribeAddressAll', function() {
    ...
    it('will unsubscribe emitter for all addresses', function() {
      ...
      var emitter2 = new EventEmitter();
        atbcoind.subscriptions.address['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br'] = [emitter1, emitter2];
        atbcoind.subscriptions.address['1Cj4UZWnGWAJH1CweTMgPLQMn26WRMfXmo'] = [emitter1, emitter2];
        atbcoind.subscriptions.address['mgY65WSfEmsyYaYPQaXhmXMeBhwp4EcsQW'] = [emitter2];
        atbcoind.subscriptions.address['3CMNFxN1oHBc4R1EpboAL5yzHGgE611Xou'] = [emitter1];
        atbcoind.unsubscribeAddress(emitter1);
        atbcoind.subscriptions.address['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br'].length.should.equal(1);
        atbcoind.subscriptions.address['1Cj4UZWnGWAJH1CweTMgPLQMn26WRMfXmo'].length.should.equal(1);
        atbcoind.subscriptions.address['mgY65WSfEmsyYaYPQaXhmXMeBhwp4EcsQW'].length.should.equal(1);
      should.not.exist(atbcoind.subscriptions.address['3CMNFxN1oHBc4R1EpboAL5yzHGgE611Xou']);
    });
  });
  ...
  describe('#_getDefaultConfig', function() {
    it('will generate config file from defaults', function() {
      var atbcoind = new BitcoinService(baseConfig);
      var config = atbcoind._getDefaultConfig();
  ...
  describe('#_loadSpawnConfiguration', function() {
    ...
    it('will parse a bitcoin.conf file', function() {
      var TestBitcoin = proxyquire('../../lib/services/atbcoind', {
        fs: {
          readFileSync: readFileSync,
      ...
      var atbcoind = new TestBitcoin(baseConfig);
        atbcoind.options.spawn.datadir = '/tmp/.bitcoin';
      var node = {};
        atbcoind._loadSpawnConfiguration(node);
      should.exist(atbcoind.spawn.config);
        atbcoind.spawn.config.should.deep.equal({
        addressindex: 1,
        checkblocks: 144,
        dbcache: 8192,
    ...
    it('will expand relative datadir to absolute path', function() {
      ...
      var config = {
        node: {
          network: bitcore.Networks.testnet,
          configPath: '/tmp/.bitcore/atbcore-node.json'
        },
        spawn: {
          datadir: './data',
          exec: 'testpath'
      ...
      var atbcoind = new TestBitcoin(config);
        atbcoind.options.spawn.datadir = './data';
      var node = {};
        atbcoind._loadSpawnConfiguration(node);
        atbcoind.options.spawn.datadir.should.equal('/tmp/.bitcore/data');
    });
    it('should throw an exception if txindex isn\'t enabled in the configuration', function() {
      ...
      var atbcoind = new TestBitcoin(baseConfig);
      (function() {
        atbcoind._loadSpawnConfiguration({datadir: './test'});
    ...
    it('should NOT set https options if node https options are set', function() {
      ...
      var atbcoind = new TestBitcoin(config);
      atbcoind.options.spawn.datadir = '/tmp/.atbcoin';
      var node = {};
      atbcoind._loadSpawnConfiguration(node);
    });
  });
  ...
  describe('#_checkConfigIndexes', function() {
    ...
    it('should warn the user if reindex is set to 1 in the bitcoin.conf file', function() {
      ...
      var node = {};
      atbcoind._checkConfigIndexes(config, node);
  ...
  describe('#_resetCaches', function() {
    it('will reset LRU caches', function() {
      ...
      for (var i = 0; i < 10; i++) {
        ...
        atbcoind.transactionDetailedCache.set(keys[i], {});
        atbcoind.utxosCache.set(keys[i], {});
        atbcoind.txidsCache.set(keys[i], {});
        atbcoind.balanceCache.set(keys[i], {});
        atbcoind.summaryCache.set(keys[i], {});
      }
      atbcoind._resetCaches();
      should.equal(atbcoind.transactionDetailedCache.get(keys[0]), undefined);
      should.equal(atbcoind.utxosCache.get(keys[0]), undefined);
      should.equal(atbcoind.txidsCache.get(keys[0]), undefined);
      should.equal(atbcoind.balanceCache.get(keys[0]), undefined);
      should.equal(atbcoind.summaryCache.get(keys[0]), undefined);
    });
  });
  ...
  describe('#_tryAllClients', function() {
    it('will retry for each node client', function(done) {
      ...
      atbcoind._tryAllClients(function(client, next) {
        client.getInfo(next);
      }, function(err) {
        if (err) {
          return done(err);
        }
        atbcoind.nodes[0].client.getInfo.callCount.should.equal(1);
        atbcoind.nodes[1].client.getInfo.callCount.should.equal(1);
        atbcoind.nodes[2].client.getInfo.callCount.should.equal(1);
        done();
      });
    });
    it('will start using the current node index (round-robin)', function(done) {
      ...
      atbcoind._tryAllClients(function(client, next) {
        ...
        atbcoind.nodes[1].client.getInfo.callCount.should.equal(1);
        atbcoind.nodes[2].client.getInfo.callCount.should.equal(1);
        atbcoind.nodesIndex.should.equal(2);
        done();
      });
    });
  ...
  describe('#_wrapRPCError', function() {
    it('will convert atbcoind-rpc error object into JavaScript error', function() {
      var atbcoind = new BitcoinService(baseConfig);
      var error = atbcoind._wrapRPCError({message: 'Test error', code: -1});
  ...
  describe('#_initChain', function() {
    ...
    it('will set height and genesis buffer', function(done) {
      ...
      atbcoind.getRawBlock = sinon.stub().callsArgWith(1, null, genesisBuffer);
      atbcoind.nodes.push({
        client: {
      ...
      atbcoind._initChain(function() {
        log.info.callCount.should.equal(1);
        atbcoind.getRawBlock.callCount.should.equal(1);
        atbcoind.getRawBlock.args[0][0].should.equal('genesishash');
        atbcoind.height.should.equal(5000);
        atbcoind.genesisBuffer.should.equal(genesisBuffer);
        done();
      });
    });
    it('it will handle error from getBestBlockHash', function(done) {
      ...
      atbcoind._initChain(function(err) {
        err.should.be.instanceOf(Error);
        done();
      });
    });
    ...
    it('it will handle error from getRawBlock', function(done) {
      ...
      atbcoind.getRawBlock = sinon.stub().callsArgWith(1, new Error('test'));
      atbcoind._initChain(function(err) {
        err.should.be.instanceOf(Error);
        done();
      });
  ...
  describe('#_getDefaultConf', function() {
    ...
    it('will get default rpc port for livenet', function() {
      ...
      var atbcoind = new BitcoinService(config);
      atbcoind._getDefaultConf().rpcport.should.equal(8332);
    });
    it('will get default rpc port for testnet', function() {
      ...
      var atbcoind = new BitcoinService(config);
      atbcoind._getDefaultConf().rpcport.should.equal(18332);
    });
  ...
  describe('#_getNetworkConfigPath', function() {
    ...
    it('will get default config path for livenet', function() {
      ...
      var atbcoind = new BitcoinService(config);
      should.equal(atbcoind._getNetworkConfigPath(), undefined);
    });
    it('will get default rpc port for testnet', function() {
      var config = {
        node: {
      ...
      var atbcoind = new BitcoinService(config);
      atbcoind._getNetworkConfigPath().should.equal('testnet3/bitcoin.conf');
    });
    it('will get default rpc port for regtest', function() {
      bitcore.Networks.enableRegtest();
      var config = {
      ...
      var atbcoind = new BitcoinService(config);
      atbcoind._getNetworkConfigPath().should.equal('regtest/bitcoin.conf');
    });
  });
  ...
  describe('#_getNetworkOption', function() {
    ...
    it('return --testnet for testnet', function() {
      var atbcoind = new BitcoinService(baseConfig);
      atbcoind.node.network = bitcore.Networks.testnet;
      atbcoind._getNetworkOption().should.equal('--testnet');
    });
    it('return --regtest for testnet', function() {
      var atbcoind = new BitcoinService(baseConfig);
      atbcoind.node.network = bitcore.Networks.testnet;
      bitcore.Networks.enableRegtest();
      atbcoind._getNetworkOption().should.equal('--regtest');
    });
    it('return undefined for livenet', function() {
      var atbcoind = new BitcoinService(baseConfig);
      atbcoind.node.network = bitcore.Networks.livenet;
      bitcore.Networks.enableRegtest();
      should.equal(atbcoind._getNetworkOption(), undefined);
    });
  });
  ...
  describe('#_zmqBlockHandler', function() {
    it('will emit block', function(done) {
      ...
      atbcoind._rapidProtectedUpdateTip = sinon.stub();
      atbcoind.on('block', function(block) {
        block.should.equal(message);
        done();
      });
      atbcoind._zmqBlockHandler(node, message);
    });
    ...
    it('will call function to update tip', function() {
      ...
      atbcoind._rapidProtectedUpdateTip = sinon.stub();
      atbcoind._zmqBlockHandler(node, message);
      atbcoind._rapidProtectedUpdateTip.callCount.should.equal(1);
      atbcoind._rapidProtectedUpdateTip.args[0][0].should.equal(node);
      atbcoind._rapidProtectedUpdateTip.args[0][1].should.equal(message);
    });
    it('will emit to subscribers', function(done) {
      ...
      atbcoind._rapidProtectedUpdateTip = sinon.stub();
      var emitter = new EventEmitter();
      atbcoind.subscriptions.hashblock.push(emitter);
      emitter.on('atbcoind/hashblock', function(blockHash) {
  ...
  describe('#_rapidProtectedUpdateTip', function() {
    it('will limit tip updates with rapid calls', function(done) {
      ...
      var count = 0;
      function repeat() {
        atbcoind._rapidProtectedUpdateTip(node, message);
        count++;
        if (count < 50) {
          repeat();
        }
  ...
  describe('#_updateTip', function() {
    ...
    it('log and emit rpc error from get block', function(done) {
      var atbcoind = new BitcoinService(baseConfig);
      atbcoind.syncPercentage = sinon.stub();
      ...
      atbcoind._updateTip(node, message);
    });
    it('emit synced if percentage is 100', function(done) {
      var atbcoind = new BitcoinService(baseConfig);
      atbcoind.syncPercentage = sinon.stub().callsArgWith(0, null, 100);
      atbcoind.on('synced', function() {
        done();
      });
      var node = {
        client: {
    ...
    it('NOT emit synced if percentage is less than 100', function(done) {
      var atbcoind = new BitcoinService(baseConfig);
      atbcoind.syncPercentage = sinon.stub().callsArgWith(0, null, 99);
      atbcoind.on('synced', function() {
    ...
    it('log and emit error from syncPercentage', function(done) {
      var atbcoind = new BitcoinService(baseConfig);
      atbcoind.syncPercentage = sinon.stub().callsArgWith(0, new Error('test'));
      atbcoind.on('error', function(err) {
        log.error.callCount.should.equal(1);
        err.message.should.equal('test');
        done();
    ...
    it('reset caches and set height', function(done) {
      var atbcoind = new BitcoinService(baseConfig);
      atbcoind.syncPercentage = sinon.stub();
      atbcoind._resetCaches = sinon.stub();
      atbcoind.on('tip', function(height) {
        atbcoind._resetCaches.callCount.should.equal(1);
        height.should.equal(10);
        atbcoind.height.should.equal(10);
        done();
      });
      var node = {
        client: {
    ...
    it('will not call syncPercentage if node is stopping', function(done) {
      ...
      atbcoind.on('tip', function() {
        atbcoind.syncPercentage.callCount.should.equal(0);
        done();
      });
      atbcoind._updateTip(node, message);
    });
  ...
  describe('#_getAddressesFromTransaction', function() {
    it('will get results using bitcore.Transaction', function() {
      ...
      tx.sign(privkey);
      var addresses = atbcoind._getAddressesFromTransaction(tx);
  ...
  describe('#_notifyAddressTxidSubscribers', function() {
    it('will emit event if matching addresses', function(done) {
      ...
      atbcoind._getAddressesFromTransaction = sinon.stub().returns([address]);
      var emitter = new EventEmitter();
      atbcoind.subscriptions.address[address] = [emitter];
      var txid = '46f24e0c274fc07708b781963576c4c5d5625d926dbb0a17fa865dcd9fe58ea0';
      var transaction = {};
      emitter.on('atbcoind/addresstxid', function(data) {
        data.address.should.equal(address);
        data.txid.should.equal(txid);
        done();
      });
      sinon.spy(emitter, 'emit');
      atbcoind._notifyAddressTxidSubscribers(txid, transaction);
  ...
  describe('#_zmqTransactionHandler', function() {
    it('will emit to subscribers', function(done) {
      ...
      var emitter = new EventEmitter();
      atbcoind.subscriptions.rawtransaction.push(emitter);
      emitter.on('atbcoind/rawtransaction', function(hex) {
        hex.should.be.a('string');
        hex.should.equal(expectedBuffer.toString('hex'));
        done();
      });
      var node = {};
      atbcoind._zmqTransactionHandler(node, expectedBuffer);
    });
    it('will NOT emit to subscribers more than once for the same tx', function(done) {
      ...
      var emitter = new EventEmitter();
      atbcoind.subscriptions.rawtransaction.push(emitter);
      emitter.on('atbcoind/rawtransaction', function() {
        done();
      });
      var node = {};
      atbcoind._zmqTransactionHandler(node, expectedBuffer);
    ...
    it('will emit "tx" event', function(done) {
      var atbcoind = new BitcoinService(baseConfig);
      var expectedBuffer = new Buffer(txhex, 'hex');
      atbcoind.on('tx', function(buffer) {
  ...
  describe('#_checkSyncedAndSubscribeZmqEvents', function() {
    ...
    it('log errors, update tip and subscribe to zmq events', function(done) {
      var atbcoind = new BitcoinService(baseConfig);
      atbcoind._updateTip = sinon.stub();
      atbcoind._subscribeZmqEvents = sinon.stub();
      var blockEvents = 0;
      atbcoind.on('block', function() {
        blockEvents++;
      });
      var getBestBlockHash = sinon.stub().callsArgWith(0, null, {
      ...
      atbcoind._checkSyncedAndSubscribeZmqEvents(node);
      setTimeout(function() {
        log.error.callCount.should.equal(2);
        blockEvents.should.equal(11);
        atbcoind._updateTip.callCount.should.equal(11);
        atbcoind._subscribeZmqEvents.callCount.should.equal(1);
        done();
      }, 200);
    });
  ...
  describe('#_subscribeZmqEvents', function() {
    it('will call subscribe on zmq socket', function() {
      ...
      atbcoind._subscribeZmqEvents(node);
    ...
    it('will call relevant handler for rawtx topics', function(done) {
      var atbcoind = new BitcoinService(baseConfig);
      atbcoind._zmqTransactionHandler = sinon.stub();
      var node = {
      ...
      node.zmqSubSocket.on('message', function() {
        atbcoind._zmqTransactionHandler.callCount.should.equal(1);
        done();
      });
    ...
    it('will call relevant handler for hashblock topics', function(done) {
      var atbcoind = new BitcoinService(baseConfig);
      atbcoind._zmqBlockHandler = sinon.stub();
      var node = {
        zmqSubSocket: new EventEmitter()
      };
      node.zmqSubSocket.subscribe = sinon.stub();
      atbcoind._subscribeZmqEvents(node);
      node.zmqSubSocket.on('message', function() {
        atbcoind._zmqBlockHandler.callCount.should.equal(1);
        done();
      });
    ...
    it('will ignore unknown topic types', function(done) {
      ...
      atbcoind._subscribeZmqEvents(node);
      node.zmqSubSocket.on('message', function() {
        atbcoind._zmqBlockHandler.callCount.should.equal(0);
        atbcoind._zmqTransactionHandler.callCount.should.equal(0);
        done();
      });
  ...
  describe('#_initZmqSubSocket', function() {
    it('will setup zmq socket', function() {
      ...
      var atbcoind = new BitcoinService(baseConfig);
      var node = {};
      atbcoind._initZmqSubSocket(node, 'url');
  ...
  describe('#_checkReindex', function() {
    ...
    it('give error from client getblockchaininfo', function(done) {
      ...
      atbcoind._checkReindex(node, function(err) {
        should.exist(err);
    ...
    it('will wait until sync is 100 percent', function(done) {
      ...
      atbcoind._checkReindex(node, function() {
  ...
  describe('#_loadTipFromNode', function() {
    ...
    it('will give rpc from client getbestblockhash', function(done) {
      ...
      atbcoind._loadTipFromNode(node, function(err) {
        err.should.be.instanceof(Error);
    ...
    it('will set height and emit tip', function(done) {
      ...
      atbcoind.on('tip', function(height) {
        ...
        atbcoind.height.should.equal(100);
        done();
      });
      atbcoind._loadTipFromNode(node, function(err) {
        if (err) {
  ...
  describe('#_stopSpawnedProcess', function() {
    ...
    it('it will kill process and resume', function(done) {
      ...
      var TestBitcoinService = proxyquire('../../lib/services/atbcoind', {
        fs: {
          readFile: readFile
        }
      });
      var atbcoind = new TestBitcoinService(baseConfig);
      atbcoind.spawnStopTime = 1;
      atbcoind._process = {};
      atbcoind._process.kill = sinon.stub();
      atbcoind._stopSpawnedBitcoin(function(err) {
        if (err) {
          return done(err);
        }
        atbcoind._process.kill.callCount.should.equal(1);
        log.warn.callCount.should.equal(1);
        done();
      });
    });
    it('it will attempt to kill process and resume', function(done) {
      ...
      error2.code = 'ESRCH';
      atbcoind._process.kill = sinon.stub().throws(error2);
      atbcoind._stopSpawnedBitcoin(function(err) {
        if (err) {
          return done(err);
        }
  ...
  describe('#_spawnChildProcess', function() {
    ...
    it('will give error from spawn config', function(done) {
      var atbcoind = new BitcoinService(baseConfig);
      atbcoind._loadSpawnConfiguration = sinon.stub();
      atbcoind._loadSpawnConfiguration = sinon.stub().throws(new Error('test'));
      atbcoind._spawnChildProcess(function(err) {
        err.should.be.instanceof(Error);
        err.message.should.equal('test');
        done();
      });
    ...
    it('will give error from stopSpawnedBitcoin', function() {
      var atbcoind = new BitcoinService(baseConfig);
      atbcoind._loadSpawnConfiguration = sinon.stub();
      atbcoind._stopSpawnedBitcoin = sinon.stub().callsArgWith(0, new Error('test'));
      atbcoind._spawnChildProcess(function(err) {
        err.should.be.instanceOf(Error);
        err.message.should.equal('test');
      });
    ...
    it('will exit spawn if shutdown', function() {
      ...
      var atbcoind = new TestBitcoinService(config);
      atbcoind.spawn = {};
      atbcoind._loadSpawnConfiguration = sinon.stub();
      atbcoind._stopSpawnedBitcoin = sinon.stub().callsArgWith(0, null);
      atbcoind.node.stopping = true;
      atbcoind._spawnChildProcess(function(err) {
        err.should.be.instanceOf(Error);
    ...
    it('will include network with spawn command and init zmq/rpc on node', function(done) {
      ...
      atbcoind._loadSpawnConfiguration = sinon.stub();
      atbcoind.spawn = {};
      atbcoind.spawn.exec = 'testexec';
      atbcoind.spawn.configPath = 'testdir/bitcoin.conf';
      atbcoind.spawn.datadir = 'testdir';
      atbcoind.spawn.config = {};
      atbcoind.spawn.config.rpcport = 20001;
      atbcoind.spawn.config.rpcuser = 'bitcoin';
      atbcoind.spawn.config.rpcpassword = 'password';
      atbcoind.spawn.config.zmqpubrawtx = 'tcp://127.0.0.1:30001';

      atbcoind._loadTipFromNode = sinon.stub().callsArgWith(1, null);
      atbcoind._initZmqSubSocket = sinon.stub();
      atbcoind._checkSyncedAndSubscribeZmqEvents = sinon.stub();
      atbcoind._checkReindex = sinon.stub().callsArgWith(1, null);
      atbcoind._spawnChildProcess(function(err, node) {
        ...
        atbcoind._loadTipFromNode.callCount.should.equal(1);
        atbcoind._initZmqSubSocket.callCount.should.equal(1);
        should.exist(atbcoind._initZmqSubSocket.args[0][0].client);
        atbcoind._initZmqSubSocket.args[0][1].should.equal('tcp://127.0.0.1:30001');
        atbcoind._checkSyncedAndSubscribeZmqEvents.callCount.should.equal(1);
        should.exist(atbcoind._checkSyncedAndSubscribeZmqEvents.args[0][0].client);
        should.exist(node);
        should.exist(node.client);
        done();
      });
    });
    it('will respawn atbcoind spawned process', function(done) {
      var process = new EventEmitter();
      var spawn = sinon.stub().returns(process);
      var TestBitcoinService = proxyquire('../../lib/services/atbcoind', {
        fs: {
      ...
      atbcoind._loadSpawnConfiguration = sinon.stub();
      atbcoind.spawn = {};
      atbcoind.spawn.exec = 'atbcoind';
      atbcoind.spawn.datadir = '/tmp/atbcoin';
      atbcoind.spawn.configPath = '/tmp/atbcoin/atbcoin.conf';
      atbcoind.spawn.config = {};
      atbcoind.spawnRestartTime = 1;
      atbcoind._loadTipFromNode = sinon.stub().callsArg(1);
      atbcoind._initZmqSubSocket = sinon.stub();
      atbcoind._checkReindex = sinon.stub().callsArg(1);
      atbcoind._checkSyncedAndSubscribeZmqEvents = sinon.stub();
      atbcoind._stopSpawnedBitcoin = sinon.stub().callsArg(0);
      sinon.spy(atbcoind, '_spawnChildProcess');
      atbcoind._spawnChildProcess(function(err) {
        if (err) {
          return done(err);
        }
        process.once('exit', function() {
          setTimeout(function() {
            atbcoind._spawnChildProcess.callCount.should.equal(2);
            done();
          }, 5);
        });
        process.emit('exit', 1);
    ...
    it('will emit error during respawn', function(done) {
      ...
      atbcoind._spawnChildProcess(function(err) {
        ...
        atbcoind._spawnChildProcess = sinon.stub().callsArgWith(0, new Error('test'));
        atbcoind.on('error', function(err) {
          err.should.be.instanceOf(Error);
          err.message.should.equal('test');
          done();
    ...
    it('will NOT respawn atbcoind spawned process if shutting down', function(done) {
      var process = new EventEmitter();
      var spawn = sinon.stub().returns(process);
      var TestBitcoinService = proxyquire('../../lib/services/atbcoind', {
        fs: {
      ...
      atbcoind._spawnChildProcess(function(err) {
        ...
        process.once('exit', function() {
          setTimeout(function() {
            atbcoind._spawnChildProcess.callCount.should.equal(1);
            done();
          }, 5);
        });
        process.emit('exit', 1);
    ...
    it('will give error after 60 retries', function(done) {
      ...
      var atbcoind = new TestBitcoinService(baseConfig);
      atbcoind.startRetryInterval = 1;
      atbcoind._loadSpawnConfiguration = sinon.stub();
      atbcoind.spawn = {};
      atbcoind.spawn.exec = 'testexec';
      atbcoind.spawn.configPath = 'testdir/atbcoin.conf';
      atbcoind.spawn.datadir = 'testdir';
      atbcoind.spawn.config = {};
      atbcoind.spawn.config.rpcport = 20001;
      atbcoind.spawn.config.rpcuser = 'bitcoin';
      atbcoind.spawn.config.rpcpassword = 'password';
      atbcoind.spawn.config.zmqpubrawtx = 'tcp://127.0.0.1:30001';
      atbcoind._loadTipFromNode = sinon.stub().callsArgWith(1, new Error('test'));
      atbcoind._spawnChildProcess(function(err) {
        atbcoind._loadTipFromNode.callCount.should.equal(60);
        err.should.be.instanceof(Error);
        done();
      });
    });
    it('will give error from check reindex', function(done) {
      ...
      atbcoind._initZmqSubSocket = sinon.stub();
      atbcoind._checkSyncedAndSubscribeZmqEvents = sinon.stub();
      atbcoind._checkReindex = sinon.stub().callsArgWith(1, new Error('test'));
  ...
  describe('#_connectProcess', function() {
    it('will give error if connecting while shutting down', function(done) {
      ...
      var atbcoind = new BitcoinService(config);
      atbcoind.node.stopping = true;
      atbcoind.startRetryInterval = 100;
      atbcoind._loadTipFromNode = sinon.stub();
      atbcoind._connectProcess({}, function(err) {
        err.should.be.instanceof(Error);
        err.message.should.match(/Stopping while trying to connect/);
        atbcoind._loadTipFromNode.callCount.should.equal(0);
        done();
      });
    });
    it('will give error from loadTipFromNode after 60 retries', function(done) {
      ...
      atbcoind.startRetryInterval = 1;
      var config = {};
      atbcoind._connectProcess(config, function(err) {
        err.should.be.instanceof(Error);
        atbcoind._loadTipFromNode.callCount.should.equal(60);
        done();
      });
    ...
    it('will init zmq/rpc on node', function(done) {
      ...
      atbcoind._loadTipFromNode = sinon.stub().callsArgWith(1, null);
      var config = {};
      atbcoind._connectProcess(config, function(err, node) {
        should.not.exist(err);
        atbcoind._loadTipFromNode.callCount.should.equal(1);
        atbcoind._initZmqSubSocket.callCount.should.equal(1);
        atbcoind._loadTipFromNode.callCount.should.equal(1);
  ...
  describe('#start', function() {
    ...
    it('will give error from connectProcess', function(done) {
      var atbcoind = new BitcoinService(baseConfig);
      atbcoind._connectProcess = sinon.stub().callsArgWith(1, new Error('test'));
      atbcoind.options = {
        connect: [
          {}
        ]
      ...
      atbcoind.start(function(err) {
        atbcoind._connectProcess.callCount.should.equal(1);
        err.should.be.instanceof(Error);
        err.message.should.equal('test');
        done();
      });
    ...
    it('will push node from spawnChildProcess', function(done) {
      var atbcoind = new BitcoinService(baseConfig);
      var node = {};
      atbcoind._initChain = sinon.stub().callsArg(0);
      atbcoind._spawnChildProcess = sinon.stub().callsArgWith(0, null, node);
      atbcoind.options = {
        spawn: {}
      };
      atbcoind.start(function(err) {
        should.not.exist(err);
        atbcoind.nodes.length.should.equal(1);
        done();
      });
    });
    it('will push node from connectProcess', function(done) {
      ...
      atbcoind._initChain = sinon.stub().callsArg(0);
      var nodes = [{}];
      atbcoind._connectProcess = sinon.stub().callsArgWith(1, null, nodes);
      atbcoind.options = {
        connect: [
          {}
        ]
  ...
  describe('#isSynced', function() {
    it('will give error from syncPercentage', function(done) {
      var atbcoind = new BitcoinService(baseConfig);
      atbcoind.syncPercentage = sinon.stub().callsArgWith(0, new Error('test'));
      atbcoind.isSynced(function(err) {
        should.exist(err);
        err.message.should.equal('test');
        done();
      });
    ...
    it('will give "true" if percentage is 100.00', function(done) {
      var atbcoind = new BitcoinService(baseConfig);
      atbcoind.syncPercentage = sinon.stub().callsArgWith(0, null, 100.00);
      atbcoind.isSynced(function(err, synced) {
        if (err) {
          return done(err);
        }
    ...
    it('will give "true" if percentage is 99.98', function(done) {
      var atbcoind = new BitcoinService(baseConfig);
      atbcoind.syncPercentage = sinon.stub().callsArgWith(0, null, 99.98);
      atbcoind.isSynced(function(err, synced) {
        if (err) {
          return done(err);
        }
    ...
    it('will give "false" if percentage is 99.49', function(done) {
      var atbcoind = new BitcoinService(baseConfig);
      atbcoind.syncPercentage = sinon.stub().callsArgWith(0, null, 99.49);
      atbcoind.isSynced(function(err, synced) {
        if (err) {
          return done(err);
        }
    ...
    it('will give "false" if percentage is 1', function(done) {
      var atbcoind = new BitcoinService(baseConfig);
      atbcoind.syncPercentage = sinon.stub().callsArgWith(0, null, 1);
      atbcoind.isSynced(function(err, synced) {
        if (err) {
          return done(err);
        }
  ...
  describe('#syncPercentage', function() {
    it('will give rpc error', function(done) {
      ...
      atbcoind.syncPercentage(function(err) {
        should.exist(err);
    ...
    it('will call client getInfo and give result', function(done) {
      ...
      atbcoind.syncPercentage(function(err, percentage) {
        if (err) {
          return done(err);
        }
  ...
  describe('#_normalizeAddressArg', function() {
    it('will turn single address into array', function() {
      var atbcoind = new BitcoinService(baseConfig);
      var args = atbcoind._normalizeAddressArg('address');
      args.should.deep.equal(['address']);
    });
    it('will keep an array as an array', function() {
      var atbcoind = new BitcoinService(baseConfig);
      var args = atbcoind._normalizeAddressArg(['address', 'address']);
  ...
  describe('#getAddressBalance', function() {
    it('will give rpc error', function(done) {
      ...
      var address = '1Cj4UZWnGWAJH1CweTMgPLQMn26WRMfXmo';
      var options = {};
      atbcoind.getAddressBalance(address, options, function(err) {
        err.should.be.instanceof(Error);
        done();
      });
    });
    it('will give balance', function(done) {
      ...
      var address = '1Cj4UZWnGWAJH1CweTMgPLQMn26WRMfXmo';
      var options = {};
      atbcoind.getAddressBalance(address, options, function(err, data) {
        if (err) {
          return done(err);
        }
        data.balance.should.equal(10000);
        data.received.should.equal(100000);
        atbcoind.getAddressBalance(address, options, function(err, data2) {
          if (err) {
            return done(err);
          }
  ...
  describe('#getAddressUnspentOutputs', function() {
    it('will give rpc error', function(done) {
      ...
      var address = '1Cj4UZWnGWAJH1CweTMgPLQMn26WRMfXmo';
      atbcoind.getAddressUnspentOutputs(address, options, function(err) {
        should.exist(err);
        err.should.be.instanceof(errors.RPCError);
        done();
      });
    ...
    it('will give results from client getaddressutxos', function(done) {
      ...
      var address = '1Cj4UZWnGWAJH1CweTMgPLQMn26WRMfXmo';
      atbcoind.getAddressUnspentOutputs(address, options, function(err, utxos) {
        if (err) {
          return done(err);
        }
  ...
  describe('#_getBalanceFromMempool', function() {
    it('will sum satoshis', function() {
      ...
      var sum = atbcoind._getBalanceFromMempool(deltas);
  ...
  describe('#_getTxidsFromMempool', function() {
    it('will filter to txids', function() {
      ...
      var txids = atbcoind._getTxidsFromMempool(deltas);
  ...
  describe('#_getHeightRangeQuery', function() {
    it('will detect range query', function() {
      ...
      var rangeQuery = atbcoind._getHeightRangeQuery(options);
    ...
    it('will get range properties', function() {
      ...
      var clone = {};
      atbcoind._getHeightRangeQuery(options, clone);
    ...
    it('will throw error with invalid range', function() {
      ...
      (function() {
        atbcoind._getHeightRangeQuery(options);
  ...
  describe('#getAddressTxids', function() {
    it('will give error from _getHeightRangeQuery', function(done) {
      var atbcoind = new BitcoinService(baseConfig);
      atbcoind._getHeightRangeQuery = sinon.stub().throws(new Error('test'));
      atbcoind.getAddressTxids('address', {}, function(err) {
        err.should.be.instanceOf(Error);
        err.message.should.equal('test');
        done();
      });
    ...
    it('will give rpc error from mempool query', function() {
      ...
      var options = {};
      var address = '1Cj4UZWnGWAJH1CweTMgPLQMn26WRMfXmo';
      atbcoind.getAddressTxids(address, options, function(err) {
        should.exist(err);
        err.should.be.instanceof(errors.RPCError);
      });
    });
    ...
    it('will get txid results', function(done) {
      ...
      var address = '1Cj4UZWnGWAJH1CweTMgPLQMn26WRMfXmo';
      atbcoind.getAddressTxids(address, options, function(err, txids) {
        if (err) {
          return done(err);
        }
    ...
    it('will get txid results from cache and live mempool', function(done) {
      ...
      var address = '1Cj4UZWnGWAJH1CweTMgPLQMn26WRMfXmo';
      atbcoind.getAddressTxids(address, {queryMempool: false}, function(err, txids) {
        if (err) {
          return done(err);
        }
        getAddressTxids.callCount.should.equal(1);
        ...
        atbcoind.getAddressTxids(address, {queryMempool: true}, function(err, txids) {
          if (err) {
            return done(err);
          }
          getAddressTxids.callCount.should.equal(1);
          ...
          atbcoind.getAddressTxids(address, {queryMempoolOnly: true}, function(err, txids) {
            if (err) {
              return done(err);
            }
            getAddressTxids.callCount.should.equal(1);
  ...
  describe('#_getConfirmationDetail', function() {
    ...
    it('should get 0 confirmation', function() {
      ...
      var atbcoind = new BitcoinService(baseConfig);
      atbcoind.height = 10;
      var confirmations = atbcoind._getConfirmationsDetail(tx);
  ...
  describe('#_getAddressDetailsForInput', function() {
    it('will return if missing an address', function() {
      var atbcoind = new BitcoinService(baseConfig);
      var result = {};
      atbcoind._getAddressDetailsForInput({}, 0, result, []);
    ...
    it('will only add address if it matches', function() {
      var atbcoind = new BitcoinService(baseConfig);
      var result = {};
      atbcoind._getAddressDetailsForInput({
        address: 'address1'
      }, 0, result, ['address2']);
      should.not.exist(result.addresses);
      should.not.exist(result.satoshis);
  ...
  describe('#_getAddressDetailsForOutput', function() {
    it('will return if missing an address', function() {
      var atbcoind = new BitcoinService(baseConfig);
      var result = {};
      atbcoind._getAddressDetailsForOutput({}, 0, result, []);
      should.not.exist(result.addresses);
      should.not.exist(result.satoshis);
    });
    it('will only add address if it matches', function() {
      var atbcoind = new BitcoinService(baseConfig);
      var result = {};
      atbcoind._getAddressDetailsForOutput({
        address: 'address1'
      }, 0, result, ['address2']);
      should.not.exist(result.addresses);
      should.not.exist(result.satoshis);
  ...
  describe('#_getAddressDetailsForTransaction', function() {
    it('will calculate details for the transaction', function(done) {
      ...
      var details = atbcoind._getAddressDetailsForTransaction(tx, addresses);
  ...
  describe('#_getAddressDetailedTransaction', function() {
    it('will get detailed transaction info', function(done) {
      ...
      var atbcoind = new BitcoinService(baseConfig);
      atbcoind.getDetailedTransaction = sinon.stub().callsArgWith(1, null, tx);
      atbcoind.height = 300;
      var addresses = {};
      atbcoind._getAddressDetailsForTransaction = sinon.stub().returns({
        addresses: addresses,
        satoshis: 1000,
      });
      atbcoind._getAddressDetailedTransaction(txid, {}, function(err, details) {
        if (err) {
          return done(err);
        }
    ...
    it('give error from getDetailedTransaction', function(done) {
      var txid = '46f24e0c274fc07708b781963576c4c5d5625d926dbb0a17fa865dcd9fe58ea0';
      var atbcoind = new BitcoinService(baseConfig);
      atbcoind.getDetailedTransaction = sinon.stub().callsArgWith(1, new Error('test'));
      atbcoind._getAddressDetailedTransaction(txid, {}, function(err) {
        err.should.be.instanceof(Error);
        done();
      });
    });
  ...
  describe('#_getAddressStrings', function() {
    it('will get address strings from bitcore addresses', function() {
      ...
      var atbcoind = new BitcoinService(baseConfig);
      var strings = atbcoind._getAddressStrings(addresses);
    ...
    it('will give error with unknown', function() {
      ...
      var atbcoind = new BitcoinService(baseConfig);
      (function() {
        atbcoind._getAddressStrings(addresses);
  ...
  describe('#getAddressHistory', function() {
    ...
    it('will give error with "from" and "to" range that exceeds max size', function(done) {
      var atbcoind = new BitcoinService(baseConfig);
      atbcoind.getAddressHistory(address, {from: 0, to: 51}, function(err) {
        should.exist(err);
    ...
    it('will give error with "from" and "to" order is reversed', function(done) {
      var atbcoind = new BitcoinService(baseConfig);
      atbcoind.getAddressTxids = sinon.stub().callsArgWith(2, null, []);
      atbcoind.getAddressHistory(address, {from: 51, to: 0}, function(err) {
        should.exist(err);
        err.message.match(/^\"from/);
        done();
      });
    ...
    it('will give error from _getAddressDetailedTransaction', function(done) {
      var atbcoind = new BitcoinService(baseConfig);
      atbcoind.getAddressTxids = sinon.stub().callsArgWith(2, null, ['txid']);
      atbcoind._getAddressDetailedTransaction = sinon.stub().callsArgWith(2, new Error('test'));
      atbcoind.getAddressHistory(address, {}, function(err) {
        should.exist(err);
        err.message.should.equal('test');
        done();
      });
    ...
    it('will give an error if length of addresses is too long', function(done) {
      ...
      var atbcoind = new BitcoinService(baseConfig);
      atbcoind.maxAddressesQuery = 100;
      atbcoind.getAddressHistory(addresses, {}, function(err) {
        should.exist(err);
    ...
    it('give error from getAddressTxids', function(done) {
      var atbcoind = new BitcoinService(baseConfig);
      atbcoind.getAddressTxids = sinon.stub().callsArgWith(2, new Error('test'));
      atbcoind.getAddressHistory('address', {}, function(err) {
        should.exist(err);
        err.should.be.instanceof(Error);
        err.message.should.equal('test');
    ...
    it('will paginate', function(done) {
      var atbcoind = new BitcoinService(baseConfig);
      atbcoind._getAddressDetailedTransaction = function(txid, options, callback) {
        callback(null, txid);
      };
      var txids = ['one', 'two', 'three', 'four'];
      atbcoind.getAddressTxids = sinon.stub().callsArgWith(2, null, txids);
      atbcoind.getAddressHistory('address', {from: 1, to: 3}, function(err, data) {
        if (err) {
          return done(err);
        }
  ...
  describe('#getAddressSummary', function() {
    ...
    it('will handle error from getAddressTxids', function(done) {
      ...
      atbcoind.getAddressTxids = sinon.stub().callsArgWith(2, new Error('test'));
      atbcoind.getAddressBalance = sinon.stub().callsArgWith(2, null, {});
      var address = '';
      var options = {};
      atbcoind.getAddressSummary(address, options, function(err) {
        should.exist(err);
        err.should.be.instanceof(Error);
        err.message.should.equal('test');
        done();
    ...
    it('will handle error from getAddressBalance', function(done) {
      ...
      atbcoind.getAddressTxids = sinon.stub().callsArgWith(2, null, {});
      atbcoind.getAddressBalance = sinon.stub().callsArgWith(2, new Error('test'), {});
      var address = '';
      var options = {};
      atbcoind.getAddressSummary(address, options, function(err) {
        should.exist(err);
    ...
    it('should set all properties', function(done) {
      ...
      sinon.spy(atbcoind, '_paginateTxids');
      atbcoind.getAddressTxids = sinon.stub().callsArgWith(2, null, [txid1, txid2, txid3]);
      atbcoind.getAddressBalance = sinon.stub().callsArgWith(2, null, {
        received: 30 * 1e8,
        balance: 20 * 1e8
      });
      ...
      var options = {};
      atbcoind.getAddressSummary(address, options, function(err, summary) {
        atbcoind._paginateTxids.callCount.should.equal(1);
        atbcoind._paginateTxids.args[0][1].should.equal(0);
        atbcoind._paginateTxids.args[0][2].should.equal(1000);
    ...
    it('will get from cache with noTxList', function(done) {
      ...
      atbcoind.getAddressSummary(address, options, function(err, summary) {
        ...
        atbcoind.getAddressTxids.callCount.should.equal(1);
        atbcoind.getAddressBalance.callCount.should.equal(1);
        atbcoind.getAddressSummary(address, options, function(err, summary) {
          checkSummary(summary);
          atbcoind.getAddressTxids.callCount.should.equal(1);
          atbcoind.getAddressBalance.callCount.should.equal(1);
    ...
    it('will skip querying the mempool with queryMempool set to false', function(done) {
      ...
      atbcoind.getAddressSummary(address, options, function() {
        getAddressMempool.callCount.should.equal(0);
        done();
      });
    });
    it('will give error from _paginateTxids', function(done) {
      ...
      atbcoind._paginateTxids = sinon.stub().throws(new Error('test'));
      var address = '3NbU8XzUgKyuCgYgZEKsBtUvkTm2r7Xgwj';
      var options = {
        queryMempool: false
      };
  ...
  describe('#getRawBlock', function() {
    ...
    it('will give rcp error from client getblockhash', function(done) {
      ...
      atbcoind.getRawBlock(10, function(err) {
        should.exist(err);
        err.should.be.instanceof(errors.RPCError);
        done();
      });
    ...
    it('will give rcp error from client getblock', function(done) {
      ...
      atbcoind.getRawBlock(blockhash, function(err) {
        should.exist(err);
        err.should.be.instanceof(errors.RPCError);
        done();
      });
    ...
    it('will try all nodes for getblock', function(done) {
      ...
      atbcoind.getRawBlock(blockhash, function(err, buffer) {
        if (err) {
          return done(err);
        }
        buffer.should.be.instanceof(Buffer);
    ...
    it('will get block by height', function(done) {
      ...
      atbcoind.getRawBlock(0, function(err, buffer) {
        if (err) {
          return done(err);
        }
        buffer.should.be.instanceof(Buffer);
  ...
  describe('#getBlock', function() {
    ...
    it('will give an rpc error from client getblock', function(done) {
      ...
      atbcoind.getBlock(0, function(err) {
        err.should.be.instanceof(Error);
        done();
      });
    });
    ...
    it('will getblock as bitcore object from height', function(done) {
      ...
      atbcoind.getBlock(0, function(err, block) {
        should.not.exist(err);
    ...
    it('will getblock as bitcore object', function(done) {
      ...
      atbcoind.getBlock('00000000050a6d07f583beba2d803296eb1e9d4980c4a20f206c584e89a4f02b', function(err, block) {
        should.not.exist(err);
  ...
  describe('#getBlockHashesByTimestamp', function() {
    it('should give an rpc error', function(done) {
      ...
      atbcoind.getBlockHashesByTimestamp(1441911000, 1441914000, function(err, hashes) {
        should.exist(err);
    ...
    it('should get the correct block hashes', function(done) {
      ...
      atbcoind.getBlockHashesByTimestamp(1441914000, 1441911000, function(err, hashes) {
        should.not.exist(err);
  ...
  describe('#getBlockHeader', function() {
    ...
    it('will give error from getBlockHash', function() {
      ...
      atbcoind.getBlockHeader(10, function(err) {
        err.should.be.instanceof(Error);
      });
    });
    it('it will give rpc error from client getblockheader', function() {
      ...
      atbcoind.getBlockHeader(blockhash, function(err) {
        err.should.be.instanceof(Error);
      });
    });
    it('it will give rpc error from client getblockhash', function() {
      ...
      atbcoind.getBlockHeader(0, function(err) {
        err.should.be.instanceof(Error);
      });
    });
    it('will give result from client getblockheader (from height)', function() {
      ...
      atbcoind.getBlockHeader(0, function(err, blockHeader) {
        should.not.exist(err);
    ...
    it('will give result from client getblockheader (from hash)', function() {
      ...
      atbcoind.getBlockHeader(blockhash, function(err, blockHeader) {
        should.not.exist(err);
        getBlockHash.callCount.should.equal(0);
        blockHeader.should.deep.equal(result);
      });
  ...
  describe('#_maybeGetBlockHash', function() {
    it('will not get block hash with an address', function(done) {
      ...
      atbcoind._maybeGetBlockHash('2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br', function(err, hash) {
        if (err) {
          return done(err);
        }
        getBlockHash.callCount.should.equal(0);
    ...
    it('will not get block hash with non zero-nine numeric string', function(done) {
      ...
      atbcoind._maybeGetBlockHash('109a', function(err, hash) {
        if (err) {
          return done(err);
        }
        getBlockHash.callCount.should.equal(0);
    ...
    it('will get the block hash if argument is a number', function(done) {
      ...
      atbcoind._maybeGetBlockHash(10, function(err, hash) {
        if (err) {
          return done(err);
        }
  ...
  describe('#getBlockOverview', function() {
    ...
    it('will handle error from maybeGetBlockHash', function(done) {
      var atbcoind = new BitcoinService(baseConfig);
      atbcoind._maybeGetBlockHash = sinon.stub().callsArgWith(1, new Error('test'));
      atbcoind.getBlockOverview(blockhash, function(err) {
        err.should.be.instanceOf(Error);
        done();
      });
    });
    ...
    it('will give expected result', function(done) {
      ...
      atbcoind.getBlockOverview(blockhash, function(err, blockOverview) {
        if (err) {
          return done(err);
        }
  ...
  describe('#estimateFee', function() {
    it('will give rpc error', function(done) {
      ...
      atbcoind.estimateFee(1, function(err) {
        should.exist(err);
        err.should.be.an.instanceof(errors.RPCError);
        done();
      });
    ...
    it('will call client estimateFee and give result', function(done) {
      ...
      atbcoind.estimateFee(1, function(err, feesPerKb) {
        if (err) {
          return done(err);
        }
  ...
  describe('#sendTransaction', function(done) {
    ...
    it('will give rpc error', function() {
      ...
      atbcoind.sendTransaction(txhex, function(err) {
        should.exist(err);
        err.should.be.an.instanceof(errors.RPCError);
      });
    });
    it('will send to client and get hash', function() {
      ...
      atbcoind.sendTransaction(txhex, function(err, hash) {
        if (err) {
          return done(err);
        }
    ...
    it('will send to client with absurd fees and get hash', function() {
      ...
      atbcoind.sendTransaction(txhex, {allowAbsurdFees: true}, function(err, hash) {
        if (err) {
          return done(err);
        }
        hash.should.equal(tx.hash);
    ...
    it('missing callback will throw error', function() {
      ...
      var transaction = bitcore.Transaction();
      (function() {
        atbcoind.sendTransaction(transaction);
      }).should.throw(Error);
    });
  });
  ...
  describe('#getRawTransaction', function() {
    it('will give rpc error', function(done) {
      ...
      atbcoind.getRawTransaction('txid', function(err) {
        should.exist(err);
        err.should.be.an.instanceof(errors.RPCError);
        done();
      });
    ...
    it('will try all nodes', function(done) {
      ...
      atbcoind.getRawTransaction('txid', function(err, tx) {
        if (err) {
          return done(err);
        }
        should.exist(tx);
  ...
  describe('#getTransaction', function() {
    it('will give rpc error', function(done) {
      ...
      atbcoind.getTransaction('txid', function(err) {
        should.exist(err);
        err.should.be.an.instanceof(errors.RPCError);
        done();
      });
    ...
    it('will try all nodes', function(done) {
      ...
      atbcoind.getTransaction('txid', function(err, tx) {
        if (err) {
          return done(err);
        }
        should.exist(tx);
  ...
  describe('#getDetailedTransaction', function() {
    ...
    it('should give a transaction with height and timestamp', function(done) {
      ...
      atbcoind.getDetailedTransaction(txid, function(err) {
        should.exist(err);
        err.should.be.instanceof(errors.RPCError);
        done();
      });
    ...
    it('should give a transaction with all properties', function(done) {
      ...
      atbcoind.getDetailedTransaction(txid, function(err, tx) {
        if (err) {
          return done(err);
        }
        checkTx(tx);
  ...
  describe('#getBestBlockHash', function() {
    it('will give rpc error', function(done) {
      ...
      atbcoind.getBestBlockHash(function(err) {
        should.exist(err);
        err.should.be.an.instanceof(errors.RPCError);
        done();
      });
    ...
    it('will call client getInfo and give result', function(done) {
      ...
      atbcoind.getBestBlockHash(function(err, hash) {
        if (err) {
          return done(err);
        }
        should.exist(hash);
  ...
  describe('#getSpentInfo', function() {
    it('will give rpc error', function(done) {
      ...
      atbcoind.getSpentInfo({}, function(err) {
        should.exist(err);
        err.should.be.an.instanceof(errors.RPCError);
        done();
      });
    ...
    it('will empty object when not found', function(done) {
      ...
      atbcoind.getSpentInfo({}, function(err, info) {
        should.not.exist(err);
  ...
  describe('#getInfo', function() {
    ...
    it('will call client getInfo and give result', function(done) {
      var atbcoind = new BitcoinService(baseConfig);
      atbcoind.node.getNetworkName = sinon.stub().returns('testnet');
      ...
      atbcoind.getInfo(function(err, info) {
        if (err) {
          return done(err);
        }
        should.exist(info);
  ...
  describe('#generateBlock', function() {
    it('will give rpc error', function(done) {
      ...
      atbcoind.generateBlock(10, function(err) {
        should.exist(err);
        err.should.be.an.instanceof(errors.RPCError);
        done();
      });
    ...
    it('will call client generate and give result', function(done) {
      ...
      atbcoind.generateBlock(10, function(err, hashes) {
        if (err) {
          return done(err);
        }
  ...
  describe('#stop', function() {
    ...
    it('will exit spawned process', function(done) {
      var atbcoind = new BitcoinService(baseConfig);
      atbcoind.spawn = {};
      atbcoind.spawn.process = new EventEmitter();
      atbcoind.spawn.process.kill = sinon.stub();
      atbcoind.stop(done);
      atbcoind.spawn.process.kill.callCount.should.equal(1);
      atbcoind.spawn.process.kill.args[0][0].should.equal('SIGINT');
      atbcoind.spawn.process.emit('exit', 0);
    });
    it('will give error with non-zero exit status code', function(done) {
      ...
      atbcoind.spawn.process.kill.callCount.should.equal(1);
      atbcoind.spawn.process.kill.args[0][0].should.equal('SIGINT');
      atbcoind.spawn.process.emit('exit', 1);
    });
    it('will stop after timeout', function(done) {
      var atbcoind = new BitcoinService(baseConfig);
      atbcoind.shutdownTimeout = 300;
      atbcoind.spawn = {};
      atbcoind.spawn.process = new EventEmitter();
      atbcoind.spawn.process.kill = sinon.stub();
      atbcoind.stop(function(err) {
