...
function checkConfigVersion2(fullConfig) {
  ...
  if (!datadirUndefined || addressDefined || dbDefined) {
    ...
    console.warn('\nConfiguration file is not compatible with this version. \n' +
                 'A reindex for atbcoind is necessary for this upgrade with the "reindex=1" atbcoin.conf option. \n' +
                 'There are changes necessary in both atbcoin.conf and atbcore-node.json. \n\n');
 ...
 * @param {Object} options.config - The parsed atbcore-node.json configuration file
...
function start(options) {
  ...
  fullConfig.path = path.resolve(options.path, './atbcore-node.json');
...
function loadModule(req, service) {
  try {
    // first try in the built-in atbcore-node services directory
    ...
    if (servicePackage.atbcoreNode) {
      serviceModule = service.name + '/' + servicePackage.atbcoreNode;
    }
 ...
 * @param {Node} node - The ATBcore Node instance
 */
