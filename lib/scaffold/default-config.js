...
/**
 * Will return the path and default atbcore-node configuration. It will search for the
 * configuration file in the "~/.atbcore" directory, and if it doesn't exist, it will create one
...
function getDefaultConfig(options) {
  ...
  var defaultConfigFile = path.resolve(defaultPath, './atbcore-node.json');

  if (!fs.existsSync(defaultPath)) {
    mkdirp.sync(defaultPath);
  }

  var defaultServices = ['atbcoind', 'web'];
