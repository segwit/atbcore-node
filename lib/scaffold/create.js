...
var BASE_PACKAGE = {
  description: 'A full ATBcoin node build with ATBcore',
  repository: 'https://github.com/user/project',
  license: 'MIT',
  readme: 'README.md',
  dependencies: {
    'atbcore-lib': '^' + bitcore.version,
    'atbcore-node': version
  }
};

/**
 * Will create a directory and atbcoin.conf file for Bitcoin.
...
/**
 * Will create a base ATBcore Node configuration directory and files.
...
function createConfigDirectory(options, configDir, isGlobal, done) {
  mkdirp(configDir, function(err) {
    ...
    try {
      fs.writeFileSync(configDir + '/atbcore-node.json', configJSON);
      if (!isGlobal) {
...
/**
 * Will setup a directory with a ATBcore Node directory, configuration file,
 * bitcoin configuration, and will install all necessary dependencies.
 *
 * @param {Object} options
 * @param {String} options.cwd - The current working directory
 * @param {String} options.dirname - The name of the atbcore node configuration directory
 * @param {String} options.datadir - The path to the ATBcoin datadir
...
function create(options, done) {
  ...
  async.series([
    function(next) {
      // Setup the the atbcore-node directory and configuration
