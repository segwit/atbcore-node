...
var BitcoinRPC = require('atbcoind-rpc');
...
/**
 * Provides a friendly event driven API to atbcoind in Node.js. Manages starting and
 * stopping atbcoind as a child process for application support, as well as connecting
 * to multiple atbcoind processes for server infrastructure. Results are cached in an
...
Bitcoin.prototype._initCaches = function() {
  ...
  this.transactionChainCache = LRU(100000);
...
Bitcoin.prototype.getAPIMethods = function() {
  var methods = [
    ...
    ['getChainTransaction', this, this.getChainTransaction, 1],
...
Bitcoin.prototype.subscribe = function(name, emitter) {
  ...
  log.info(emitter.remoteAddress, 'subscribe:', 'atbcoind/' + name, 'total:', this.subscriptions[name].length);
};
...
Bitcoin.prototype.unsubscribe = function(name, emitter) {
  ...
  log.info(emitter.remoteAddress, 'unsubscribe:', 'atbcoind/' + name, 'total:', this.subscriptions[name].length);
};
...
Bitcoin.prototype.subscribeAddress = function(emitter, addresses) {
  ...
  log.info(emitter.remoteAddress, 'subscribe:', 'atbcoind/addresstxid', 'total:', _.size(this.subscriptions.address));
};
...
Bitcoin.prototype.unsubscribeAddress = function(emitter, addresses) {
  ...
  log.info(emitter.remoteAddress, 'unsubscribe:', 'atbcoind/addresstxid', 'total:', _.size(this.subscriptions.address));
};
...
Bitcoin.prototype._loadSpawnConfiguration = function(node) {
  ...
  $.checkArgument(this.options.spawn, 'Please specify "spawn" in atbcoind config options');
  $.checkArgument(this.options.spawn.datadir, 'Please specify "spawn.datadir" in atbcoind config options');
  $.checkArgument(this.options.spawn.exec, 'Please specify "spawn.exec" in atbcoind config options');

  this._expandRelativeDatadir();

  var spawnOptions = this.options.spawn;
  var configPath = path.resolve(spawnOptions.datadir, './atbcoin.conf');

  log.info('Using atbcoin config file:', configPath);
...
Bitcoin.prototype._checkConfigIndexes = function(spawnConfig, node) {
  $.checkState(
    ...
    '"txindex" option is required in order to use transaction query features of atbcore-node. ' +
  ...
  $.checkState(
    ...
    '"addressindex" option is required in order to use address query features of atbcore-node. ' +
  ...
  $.checkState(
    ...
    '"spentindex" option is required in order to use spent info query features of atbcore-node. ' +
  ...
  $.checkState(
    ...
    '"server" option is required to communicate to atbcoind from atbcore. ' +
      'Please add "server=1" to your configuration and restart'
  );

  $.checkState(
    spawnConfig.zmqpubrawtx,
    '"zmqpubrawtx" option is required to get event updates from atbcoind. ' +
      'Please add "zmqpubrawtx=tcp://127.0.0.1:<port>" to your configuration and restart'
  );

  $.checkState(
    spawnConfig.zmqpubhashblock,
    '"zmqpubhashblock" option is required to get event updates from atbcoind. ' +
  ...
  $.checkState(
    ...
    '"zmqpubrawtx" and "zmqpubhashblock" are expected to the same host and port in atbcoin.conf'
  );

  if (spawnConfig.reindex && spawnConfig.reindex === 1) {
    log.warn('Reindex option is currently enabled. This means that atbcoind is undergoing a reindex. ' +
             ...
             'of atbcore-node services will start.');
    node._reindex = true;
  }
};
...
Bitcoin.prototype._getNetworkConfigPath = function() {
  var networkPath;
  if (this.node.network === bitcore.Networks.testnet) {
    networkPath = 'testnet3/atbcoin.conf';
    if (this.node.network.regtestEnabled) {
      networkPath = 'regtest/atbcoin.conf';
    }
  }
  return networkPath;
};
...
Bitcoin.prototype._zmqBlockHandler = function(node, message) {
  ...
  if (!self.zmqKnownBlocks.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.hashblock.length; i++) {
      this.subscriptions.hashblock[i].emit('atbcoind/hashblock', message.toString('hex'));
    }
  }
    ...
    if(!self.node.stopping) {
      self.syncPercentage(function(err, percentage) {
        if (err) {
          ...
          log.info('ATBcoin Height:', self.height, 'Percentage:', percentage.toFixed(2));
        }
      });
    }
  }
  ...
  for (var i = 0; i < addresses.length; i++) {
    ...
    if(this.subscriptions.address[address]) {
      ...
      for(var j = 0; j < emitters.length; j++) {
        emitters[j].emit('atbcoind/addresstxid', {
          address: address,
          txid: txid
        });
      }
...
Bitcoin.prototype._zmqTransactionHandler = function(node, message) {
  ...
  if (!self.zmqKnownTransactions.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.rawtransaction.length; i++) {
      this.subscriptions.rawtransaction[i].emit('atbcoind/rawtransaction', message.toString('hex'));
    }
  ...
  if (node._reindex) {
    interval = setInterval(function() {
      node.client.getBlockchainInfo(function(err, response) {
        ...
        log.info('ATBcoin Core Daemon Reindex Percentage: ' + percentSynced.toFixed(2));
...
Bitcoin.prototype._stopSpawnedBitcoin = function(callback) {
  var self = this;
  var spawnOptions = this.options.spawn;
  var pidPath = spawnOptions.datadir + '/atbcoind.pid';
  ...
  function stopProcess() {
    fs.readFile(pidPath, 'utf8', function(err, pid) {
      ...
      try {
        log.warn('Stopping existing spawned atbcoin process with pid: ' + pid);
...
Bitcoin.prototype._spawnChildProcess = function(callback) {
  ...
  self._stopSpawnedBitcoin(function(err) {
    ...
    log.info('Starting atbcoin process');
    ...
    async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
      ...
      if (exitShutdown) {
        return callback(new Error('Stopping while trying to spawn atbcoind.'));
      }
...
Bitcoin.prototype._connectProcess = function(config, callback) {
  ...
  async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
    ...
    if (exitShutdown) {
      return callback(new Error('Stopping while trying to connect to atbcoind.'));
    }
 ...
 * @param {String|Address|Array} addressArg - An address string, atbcore address, or array of addresses
 * @param {Object} options
 * @param {Function} callback
 */
...
Bitcoin.prototype._getHeightRangeQuery = function(options, clone) {
  if (options.start >= 0 && options.end >= 0) {
    ...
    if (clone) {
      // reverse start and end as the order in atbcore is most recent to less recent
...
/**
 * Will retrieve a block as a ATBcore object
 * @param {String|Number} block - A block hash or block height number
 * @param {Function} callback
 */
 ...
 *   flags: "proof-of-stake"//proof-of-work
 * }
...
/**
 * Will get a chain transaction as a ATBcore Transaction.
 * @param {String} txid - The transaction hash
 * @param {Function} callback
 */
Bitcoin.prototype.getChainTransaction = function(txid, callback) {
    var self = this;
    var tx = self.transactionChainCache.get(txid);
    if (tx) {
        return setImmediate(function() {
            callback(null, tx);
        });
        ...
        self._tryAllClients(function(client, done) {
            client.getRawTransaction(txid, 1, function(err, response) {
                ...
                self.transactionChainCache.set(txid, tx);
                done(null, tx);
            });
        }, callback);
    }
...
/**
 * Will get a transaction as a ATBcore Transaction. Results include the mempool.
 * @param {String} txid - The transaction hash
  ...
  if (this.spawn && this.spawn.process) {
    ...
    this.spawn.process.once('exit', function(code) {
      if (!exited) {
        exited = true;
        if (code !== 0) {
          var error = new Error('atbcoind spawned process exited with status code: ' + code);
          error.code = code;
    ...
    setTimeout(function() {
      if (!exited) {
        exited = true;
        return callback(new Error('atbcoind process did not exit'));
      }
 ...
 * @param {Function} callback
 */
Bitcoin.prototype.getSubsidy = function(height, callback) {

    var self = this;

    return self._tryAllClients(function(client, done) {
        return client.getSubsidy(height, function(err, response) {
